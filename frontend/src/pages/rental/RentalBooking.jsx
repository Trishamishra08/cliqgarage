...
const RentalBooking = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-['Outfit'] select-none overflow-hidden relative">
      {/* Fixed Nano Header */}
      <div className="fixed top-0 left-0 right-0 bg-gradient-to-r from-[#004AAD] to-[#14b8a6] z-50 px-6 py-3 flex items-center justify-between shadow-xl">
        <button 
          onClick={step === 1 ? () => navigate(-1) : prevStep}
          className="w-8 h-8 rounded-lg bg-white/10 border border-white/20 flex items-center justify-center text-white active:scale-95"
        >
          <ChevronLeft size={16} strokeWidth={2.5} />
        </button>
        <div className="text-center">
          <p className="text-[7px] font-black text-white/50 uppercase tracking-[0.4em] mb-0.5">Step {step} of 6</p>
          <h2 className="text-[9px] font-black text-white uppercase tracking-[0.2em]">Rental Flow</h2>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 2 && (
            <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4 pt-2">
               <div className="bg-white p-4 rounded-2xl shadow-xl border border-slate-100 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                     <div className="w-8 h-8 rounded-lg bg-[#004AAD]/5 flex items-center justify-center text-[#004AAD]"><MapPin size={16} /></div>
                     <div><p className="text-[7px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Pickup Location</p><p className="text-[10px] font-black text-slate-900 uppercase">{location}</p></div>
                  </div>
                  <button className="text-[7px] font-black text-[#004AAD] uppercase tracking-widest">Change</button>
               </div>
               <div className="grid gap-3">
                  {vehicles[vehicleType].map((v) => (
                    <motion.div key={v.id} whileTap={{ scale: 0.98 }} onClick={() => { setSelectedVehicle(v); nextStep(); }} className="bg-white rounded-2xl overflow-hidden shadow-lg border border-slate-50 flex">
                       <div className="w-1/3 h-24 relative overflow-hidden"><img src={v.image} className="w-full h-full object-cover" /></div>
                       <div className="flex-1 p-3 flex flex-col justify-center">
                          <h4 className="text-[11px] font-black text-slate-900 uppercase mb-0.5">{v.name}</h4>
                          <p className="text-[7px] font-bold text-slate-400 uppercase mb-2">{v.stats}</p>
                          <div className="flex justify-between items-center"><span className="text-sm font-black text-[#004AAD]">₹{v.price}<span className="text-[6px] opacity-40">/ Day</span></span><ArrowRight size={14} className="text-[#004AAD]" /></div>
                       </div>
                    </motion.div>
                  ))}
               </div>
      ...
      {/* Progress Line */}
      <div className="fixed top-[52px] left-0 right-0 h-0.5 bg-slate-100/50 z-50">
         <motion.div initial={{ width: 0 }} animate={{ width: `${(step/6) * 100}%` }} className="h-full bg-[#14b8a6] transition-all duration-700" />
      </div>
    </div>
  );
};
