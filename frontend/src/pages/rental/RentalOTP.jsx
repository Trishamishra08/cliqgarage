...
import { Phone, ArrowLeft, Bike } from 'lucide-react';
...
const RentalOTP = () => {
      ...
      if (isRegister) {
        navigate('/rental/setup', { 
          state: { 
            ...
            bikeModel: registerData?.bikeModel 
          } 
        });
      } else {
  ...
  return (
    <div className="min-h-screen bg-white font-['Roboto'] flex flex-col">
      {/* Header */}
      <div className="relative w-full bg-[#1a472a] shrink-0" style={{ height: '22vh', borderBottomLeftRadius: '50%', borderBottomRightRadius: '50%' }}>
        <button
          ...
          <form onSubmit={handleVerifyOTP} className="w-full flex flex-col items-center gap-6">
            <motion.div variants={itemVariants} className="flex gap-2 justify-center">
              {otp.map((digit, index) => (
                <input
                  ...
                  maxLength="1"
                  className="w-9 h-9 text-center text-sm font-bold border-2 border-slate-200 rounded-md focus:border-[#22c55e] focus:outline-none bg-[#F8FAFC] text-[#1a472a] transition-all"
                />
              ))}
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="text-center">
              {!canResend ? (
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                  Resend OTP in <span className="text-[#22c55e] font-black">{timer}s</span>
                </p>
              ) : (
                <button
                  type="button"
                  onClick={handleResendOTP}
                  className="text-[10px] font-black text-[#1a472a] uppercase tracking-[0.2em] border-b-2 border-[#22c55e] pb-0.5 hover:text-[#22c55e] transition-colors"
                >
                  Resend OTP
                </button>
              )}
            ...
            <motion.button
              ...
              className={`w-full py-3.5 rounded-full font-black text-xs tracking-[0.2em] uppercase transition-all duration-300 relative overflow-hidden group ${
                otp.join('').length === 6 ? 'bg-[#1a472a] text-white shadow-md' : 'bg-slate-100 text-slate-300'
              }`}
            >
